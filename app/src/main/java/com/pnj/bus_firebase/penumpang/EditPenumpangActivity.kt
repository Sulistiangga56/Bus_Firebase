...
class EditPenumpangActivity : AppCompatActivity() {
    ...
    fun setDefaultValue(): Array<Any> {
        val intkent = intent
        val defaultnik = intent.getStringExtra("nik").toString()
        val defaultnama = intent.getStringExtra("nama").toString()
        val defaulttgl_berangkat = intent.getStringExtra("tgl_berangkat").toString()
        val defaulttujuan = intent.getStringExtra("tujuan").toString()
        val defaultjenis_kelamin = intent.getStringExtra("jenis_kelamin").toString()
5
        binding.TxtEditNIK.setText(defaultnik)
        binding.TxtEditNama.setText(defaultnama)
        binding.TxtEditTglBerangkat.setText(defaulttgl_berangkat)
        binding.TxtEditTujuan.setText(defaulttujuan)

        val tgl_split = intent.getStringExtra("tgl_berangkat")
            .toString().split("-").toTypedArray()
        val defaultyear = tgl_split[0].toInt()
        val defaultmonth = tgl_split[1].toInt() - 1
        val defaultday = tgl_split[2].toInt()
        if (defaultjenis_kelamin == "Laki-Laki") {
            binding.RdnEditJKL.isChecked = true
        } else if (defaultjenis_kelamin == "Perempuan") {
            binding.RdnEditJKP.isChecked = true
        }


        val curr_penumpang = Penumpang(defaultnik, defaultnama, defaulttgl_berangkat, defaulttujuan, defaultjenis_kelamin)
        return arrayOf(defaultyear, defaultmonth, defaultday, curr_penumpang)
    }
